...
// An interface telling the methods of the game class must implement
// Assumed that any implementation of this interface will have a constructor
// that takes a single parameter "size" which tells the side length of the game grid
public interface Game {
    ...
    // We should use the PathFinder class to determine the winner
    // It should return PieceColour.NONE if the game is not over
    // It should also return PieceColour.NONE if the game is a draw
    PieceColour winner();

    // The colour of the current player (the player who will make the next move)
    // If the game is over, the output of this method is not defined
